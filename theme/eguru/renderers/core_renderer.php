...
class theme_eguru_core_renderer extends theme_boost\output\core_renderer {
    ...
    public function navbar()
    {
        ...
        $breadcrumbs = $this->getShortBreadcrumbs($items);


        $result = html_writer::start_tag('nav', ['role' => 'navigation']);
        $result .= html_writer::start_tag('ol', ['class' => 'breadcrumb']);

        foreach ($breadcrumbs as $breadcrumb) {
            $result .= html_writer::tag('li', $breadcrumb, ['class' => 'breadcrumb-item']);
        }

        $result .= html_writer::end_tag('nav');
        $result .= html_writer::end_tag('ol');

        return $result;
    }

    protected function getShortBreadcrumbs($items)
    {
        $count = count($items);
        if ($count > 4) {
            $iteration = 1;
            $lastIteration = $count - 1;
            ...
            foreach ($items as $item) {
                ...
                if ($iteration == $lastIteration) {
                    $breadcrumbs[] = $this->render(new breadcrumb_navigation_node(array(
                        'text' => '...',
                        'shorttext' => '...',
                        'key' => '...',
                        'action' => $item->action
