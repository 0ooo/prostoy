...
class theme_eguru_core_course_renderer extends core_course_renderer {
    ...
    public function frontpage_available_courses() {
        ...
        if (count($rcourseids) > 0) {
            ...
            foreach ($rcourseids as $courseid) {
                ...
                //альтернатива закомментированной выше строки
                $rowcontent .= html_writer::start_div('col-md-3 col-sm-6');
                $rowcontent .= html_writer::start_div('fp-coursebox');

                
                $rowcontent .= html_writer::start_div('fp-coursethumb');
                $rowcontent .= html_writer::start_tag('a', ['href' => $courseurl]);
                $rowcontent .= html_writer::img($imgurl, $course->fullname, ['width' => '243', 'height' => '165']);
                $rowcontent .= html_writer::end_tag('a');
                $rowcontent .= html_writer::end_div();

                $rowcontent .= html_writer::div($this->getCourseDateInfo($course));
                
                $rowcontent .= html_writer::start_div('fp-courseinfo');
                $rowcontent .= html_writer::start_tag('h5');
                $rowcontent .= html_writer::link($courseurl, $course->fullname);
                $rowcontent .= html_writer::end_tag('h5');
                $rowcontent .= html_writer::end_div();
    ...
    protected function getCourseDateInfo($course)
    {
        $content = '';

        $content .= html_writer::start_div();
        $now = time();
        $start = $course->startdate;//значение в секундах
        $end = $course->enddate;//значение в секундах

        $interval = date('d.m.Y', $start) . ' - ' . date('d.m.Y', $end);

        $diffDays = round(($start - $now) / (60 * 60 * 24));

        $floatDiff = ($start - $now) / (60 * 60 * 24);

        if ($start < $now) {
            $dateInfo = 'Курс уже начался';
        } elseif ($diffDays >= 5) {
            $dateInfo = "Старт через {$diffDays} дней";
        } elseif ($diffDays < 5 && $diffDays > 1) {
            $dateInfo = "Старт через {$diffDays} дня";
        } elseif ($diffDays == 1) {
            $dateInfo = "Старт через 1 день";
        }elseif($floatDiff > 0 && $floatDiff < 1){
            $dateInfo = "До старта менее одного дня";
        }

        //иконка календаря
        $icon = html_writer::span('', 'fa fa-calendar mr-2');

        $intervalHtml = html_writer::span($interval);

        //Вывод иконки календаря и интервала
        $content .= html_writer::div($icon . $intervalHtml, 'text-center interval');
        //Вывод текстом "Старт через .. дней"
        $content .= html_writer::div($dateInfo ?? '', 'text-center start-info');
